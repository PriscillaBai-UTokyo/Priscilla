correlationNumBar = function(input,column,resultname,ylabname){
  library(ggplot2)
  library(dplyr)
  corbar = sapply(unique(input[,column]),function(i){c(length(which(subset(input,input[,column]==i)$coefficient>0)),length(which(subset(input,input[,column]==i)$coefficient<0)))})
  corbar=t(corbar)
